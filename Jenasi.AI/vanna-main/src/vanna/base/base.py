# 提供了与 Vanna 实例交互的核心功能。VannaBase 类包含了一系列的抽象方法，这些方法需要在子类中具体实现，
# 以便与不同的数据库和语言模型（LLM）进行交互。此外，它还提供了一些具体的方法，用于生成 SQL 查询、执行 SQL、训练模型等。
...
class VannaBase(ABC):
    def __init__(self, config=None):
      #可选参数，用户可传入一个字典来配置对象的属性
        if config is None:
            config = {}

        self.config = config  #保存传入的配置字典
    ...
    def generate_sql(self, question: str, allow_llm_to_see_data=False, **kwargs) -> str:
      #参数：1.问题；2.是否允许LLM查看数据，设定为否既无法访问数据库内容；3.输入参数，可以传递不定长度的键值对
        """
        Example:
        ```python
        ...
        if self.config is not None:
            ...
            initial_prompt = None
        #获取相似问题的SQL
        question_sql_list = self.get_similar_question_sql(question, **kwargs) #检索与问题相关的SQL查询
        ddl_list = self.get_related_ddl(question, **kwargs) #检索与问题相关的数据库结构
        doc_list = self.get_related_documentation(question, **kwargs)   #检索相关的文档
        #设计提示词
